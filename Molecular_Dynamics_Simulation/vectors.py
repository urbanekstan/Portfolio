...
def triangularLattice( ttl, xi, yi, Xb, Yb, v1x, v1y, v2x, v2y, doesRotate, theta):
...
while ny * v2y < Yb:
    while nx * v1x < Xb:
        ...
        xf, yf, zf = conv2to3dim(pnewx, pnewy, Xb, Yb, doesRotate, theta, phi, sc)
        # DRAW 3 dim point
...
def honeycombLattice(ttl, xi, yi, Xb, Yb, v1x, v1y, v2x, v2y, doesRotate, theta, p):
...
def rotate3dim ( x_f, y_f, z_f, theta, phi ):
    ...
    rotate = matrix( [ [cos(theta)*cos(phi), cos(theta) * sin(phi), - sin(theta)],[-si] ] )
...
def main():
    ...
    scale = math.sqrt(3) * d_carbonBondLength
    # Ref. notebook 5/26
    #25 good for visuals
